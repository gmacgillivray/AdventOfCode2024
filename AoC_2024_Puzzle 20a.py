...
def find_shortcuts(walls, wandh, delta_time):
    ...
    for i in range(len(walls)):
        if walls[i][0] > 0 and walls[i][0] < wandh[0] and walls[i][1] > 0 and walls[i][1] < wandh[1]:
            if [walls[i][0] - 1, walls[i][1]] not in walls and [walls[i][0] + 1, walls[i][1]] not in walls:
                walls_to_pass.append(walls[i])
            elif [walls[i][0], walls[i][1] - 1] not in walls and [walls[i][0], walls[i][1] - 1] not in walls:
                walls_to_pass.append(walls[i])
