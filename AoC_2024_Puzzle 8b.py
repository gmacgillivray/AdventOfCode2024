...
def reduce_fraction(num1, num2):
    ...
    f_x = prime_factors(num1)
    f_y = prime_factors(num2)
...
for key in dish_locations:
    ...
    for i in range(len(points_list)-1):
        for j in range(i+1, len(points_list)):
            ...
            delta_x, delta_y = reduce_fraction(delta_x, delta_y)
            
            m = len(a)/abs(delta_x)
            n = len(a[0])/abs(delta_y)
            ...
            for k in range(0, p):
                anti_resonance = [points_list[i][0] - (k * delta_x), points_list[i][1] - (k * delta_y)]
            ...
            for k in range(0, p):        
                anti_resonance = [points_list[j][0] + (k * delta_x), points_list[j][1] + (k * delta_y)]                
            
                if anti_resonance[0] >= 0 and anti_resonance[0] < len(a[0]) and anti_resonance[1] >= 0 and anti_resonance[1] < len(a):
                    if [anti_resonance[0], anti_resonance[1]] not in anti_resonance_points:
                        anti_resonance_points.append([anti_resonance[0], anti_resonance[1]])

print("The number of unique anti_node locations for part b is = ", len(anti_resonance_points))
