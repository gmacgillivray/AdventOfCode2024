...
def move_robot_2(p, move, boxes_l, boxes_r, walls):
    ...
    if vector[1] == 0:
        while not found_a_wall and not found_a_space:
            if new_pos in walls:
                found_a_wall = True
            elif new_pos in boxes_l:
                if boxes_l.index([new_pos[0], new_pos[1]]) not in index_points_to_shift:
                    index_points_to_shift.append(boxes_l.index([new_pos[0], new_pos[1]]))
            elif new_pos in boxes_r:
                if boxes_r.index([new_pos[0], new_pos[1]]) not in index_points_to_shift:
                    index_points_to_shift.append(boxes_r.index([new_pos[0], new_pos[1]]))
            else:
                found_a_space = True
            new_pos = [new_pos[0] + vector[0], new_pos[1] + vector[1]]
        
        if found_a_space and not found_a_wall:
            p = [p[0] + vector[0], p[1] + vector[1]]
            for ix in index_points_to_shift:
                new_boxes_l[ix] = [boxes_l[ix][0] + vector[0], boxes_l[ix][1] + vector[1]]
                new_boxes_r[ix] = [boxes_r[ix][0] + vector[0], boxes_r[ix][1] + vector[1]]
    
    else:
        
        points_to_check = []
        points_to_check.append(new_pos)

        while not found_a_wall and not found_a_space:
            
            temp_points_to_check = []

            for i in range(len(points_to_check)):
                if points_to_check[i] in walls:
                    found_a_wall = True
                elif points_to_check[i] in boxes_l:
                    if boxes_l.index(points_to_check[i]) not in index_points_to_shift:
                        index_points_to_shift.append(boxes_l.index(points_to_check[i]))
                    if boxes_l[boxes_l.index(points_to_check[i])] not in temp_points_to_check:
                        temp_points_to_check.append(boxes_l[boxes_l.index(points_to_check[i])])
                    if boxes_r[boxes_l.index(points_to_check[i])] not in temp_points_to_check:
                        temp_points_to_check.append(boxes_r[boxes_l.index(points_to_check[i])])    
                elif points_to_check[i] in boxes_r:
                    if boxes_r.index(points_to_check[i]) not in index_points_to_shift:
                        index_points_to_shift.append(boxes_r.index(points_to_check[i]))
                    if boxes_l[boxes_r.index(points_to_check[i])] not in temp_points_to_check:
                        temp_points_to_check.append(boxes_l[boxes_r.index(points_to_check[i])])
                    if boxes_r[boxes_r.index(points_to_check[i])] not in temp_points_to_check:
                        temp_points_to_check.append(boxes_r[boxes_r.index(points_to_check[i])]) 
            
            if temp_points_to_check == []:
                    found_a_space = True

            points_to_check = []
            for point in temp_points_to_check:
                points_to_check.append([point[0] + vector[0], point[1] + vector[1]])
