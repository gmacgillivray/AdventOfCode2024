...
#                       check = 1
                        index_match.append([r, s])
                        part_number_list.append(Part_Number)
#            if check == 1:
#                Part_Number_Sum += Part_Number
for target_element in index_match:
    matching_indices = []
    for i,e in enumerate(index_match):
        if e == target_element:
            matching_indices.append(i)
    if len(matching_indices) == 2:
        Part_Number_Sum += part_number_list[matching_indices[0]] * part_number_list[matching_indices[1]]
        part_number_list[matching_indices[0]] = 0
        part_number_list[matching_indices[1]] = 0
