...
def number_sides(plot_points):
    ...
    potential_starting_next_steps = []

    
    for i in range(len(plot_points)):
        label = (plot_points[i][0],plot_points[i][1])
        if len(temp_graph_points[label]) < 4:
            for k in range(len(temp_graph_points[label])):
                potential_starting_points.append([plot_points[i][0], plot_points[i][1]])
                potential_starting_next_steps.append([temp_graph_points[label][k][0] - [plot_points[i][0]], temp_graph_points[label][k][1] - [plot_points[i][1]]])  

    for i in range(len(potential_starting_points)):

        starting_pos_indices = []
        starting_next_step_indices = []

        starting_pos = [potential_starting_points[i][0], potential_starting_points[i][1]]
        starting_next_step = [potential_starting_next_steps[i][0], potential_starting_next_steps[i][1]]                            
        starting_pos_indices = [index for index, value in enumerate(all_points) if value == starting_pos]
        starting_next_step_indices = [index for index, value in enumerate(all_nextsteps) if value == starting_next_step]
    
        for m in range(len(starting_pos_indices)):
            if starting_pos_indices[m] in starting_next_step_indices:
